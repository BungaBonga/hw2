...
int main(int argc, char** argv, char** envp) {
    ...
    //define inpunt FIFO filename
    if ((CLFIFO = malloc(len_pid + 7)) == NULL) {
        fprintf(stderr, "ERROR");
        return -1;
    }
    sprintf(CLFIFO, "./fifo%d", pid);
    ...
    sprintf(req, "%s %s", CLFIFO, argv[1]);

    umask(0);
    //create FIFO for input
    if (mkfifo(CLFIFO, 0600) < 0) {
        perror("client fifo failed");
        return errno;
    }
    //request to server 
    if ((fdout = open(SERVFIFO, O_WRONLY)) < 0) {
    ...
    if ((fdin = open(CLFIFO, O_RDONLY)) < 0) {
